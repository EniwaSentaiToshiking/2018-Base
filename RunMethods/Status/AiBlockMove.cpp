...
AiBlockMove::AiBlockMove(){
  ...
  patterns.push_back(new RunPattern(SPIN, 20, DIRECTION,-90));
  patterns.push_back(new RunPattern(LINE_TRACE, 15, DISTANCE, 20,2.0, 0.0, 0.05, 70, RIGHT));
  ...
  patterns.push_back(new RunPattern(LINE_TRACE, 20, DISTANCE, 5,3.0, 0.0, 0.05, 70, LEFT));//PIDの数値を上げて，ライン復帰を強制的にしています．
  ...
  //-----------------------------------------デジタル青解答開始------------------------------------------------
  //1.青まで走る
  patterns.push_back(new RunPattern(LINE_TRACE, 20, COLOR ,2, 2.0, 0.0, 0.05, 30, LEFT));
  //2.ちょっとでる
  ...
  if(deg_ansnum >=4){
    ...
    patterns.push_back(new RunPattern(STRAIGHT, 10, DISTANCE,5));
    patterns.push_back(new RunPattern(STRAIGHT, -10, DISTANCE,5));
    patterns.push_back(new RunPattern(SPIN, -20, COLOR,1,1.0, 0.0, 0.05, 30, LEFT));
  }//leftラインへ復帰
  patterns.push_back(new RunPattern(LINE_TRACE, 20, DISTANCE, 5,3.0, 0.0, 0.05, 70, LEFT));//PIDの数値を上げて，ライン復帰を強制的にしています．
  patterns.push_back(new RunPattern(LINE_TRACE, 20, BRIGHTNESS,40, 2.0, 0.0, 0.05, 70, LEFT));
  patterns.push_back(new RunPattern(STRAIGHT, 15, DISTANCE,6));
  ...
  //-----------------------------------------アナログ緑解答開始------------------------------------------------
//1.緑まで走る
  patterns.push_back(new RunPattern(LINE_TRACE, 15, DISTANCE ,5, 3.0, 0.0, 0.05, 30, RIGHT));//応急処置
  ...
  //-----------------------------------------デジタル黄解答開始------------------------------------------------
  //1.黄まで走る
  //patterns.push_back(new RunPattern(LINE_TRACE, 15, DISTANCE ,5, 1.0, 0.0, 0.05, 100,LEFT));//応急処置
  patterns.push_back(new RunPattern(LINE_TRACE, 20, COLOR ,4, 3.0, 0.0, 0.05, 30, LEFT));
  //2.ちょっとでる
  patterns.push_back(new RunPattern(STRAIGHT, 10, DISTANCE,5));
  //3.解答方向へ回転(if文の中)
  //ちょっと押す
  ...
  patterns.push_back(new RunPattern(LINE_TRACE, 20, DISTANCE, 5,3.0, 0.0, 0.05, 70, LEFT));//PIDの数値を上げて，ライン復帰を強制的にしています．
  patterns.push_back(new RunPattern(LINE_TRACE, 20, BRIGHTNESS,40, 2.0, 0.0, 0.05, 70, LEFT));
  patterns.push_back(new RunPattern(STRAIGHT, 10, DISTANCE,6));
  patterns.push_back(new RunPattern(SPIN, 20, DIRECTION,90));//90度回転　LINEはレフトを見たまま,交差点に入るまで
  //-----------------------------------------デジタル黄解答終了------------------------------------------------
  //3レーン目入る
  patterns.push_back(new RunPattern(LINE_TRACE, 15, DISTANCE, 20,2.0, 0.0, 0.05, 70, RIGHT));
  ...
  //-----------------------------------------アナログ赤解答開始------------------------------------------------
  //1.赤まで走る
  //patterns.push_back(new RunPattern(LINE_TRACE, 15, DISTANCE ,10, 2.0, 0.0, 0.05, 100,RIGHT));//応急処置
  patterns.push_back(new RunPattern(LINE_TRACE, 15, COLOR ,5, 2.0, 0.0, 0.05, 30, RIGHT));
  //2.ちょっとでる
  patterns.push_back(new RunPattern(STRAIGHT, 10, DISTANCE,5));
  //3.解答方向へ回転(if文の中)
  //ちょっと押す
  ...
  //-----------------------------------------デジタル赤解答開始------------------------------------------------
  //1.赤まで走る
  //patterns.push_back(new RunPattern(LINE_TRACE, 15, DISTANCE ,10, 2.0, 0.0, 0.05, 100,LEFT));//応急処置
  patterns.push_back(new RunPattern(LINE_TRACE, 20, COLOR ,5, 2.0, 0.0, 0.05, 30, LEFT));
  ...
  patterns.push_back(new RunPattern(SPIN, 20, DIRECTION,90));//90度回転　LINEはレフトを見たまま,交差点に入るまで
  //-----------------------------------------デジタル赤解答終了------------------------------------------------
  patterns.push_back(new RunPattern(LINE_TRACE, 20, DISTANCE, 5, 3.0, 0.0, 0.01, 40, RIGHT));
  patterns.push_back(new RunPattern(LINE_TRACE, 20, DISTANCE, 15, 2.0, 0.0, 0.01, 40, RIGHT));
  //AIアンサー解答終了
  setNextState();
}
